...
public class MutlColorLoadingView extends View {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (w > h) {
            offset = (w - h) / 2f;
            mLayer.set(halfOfStrokeWidth + paddingLeft + offset, halfOfStrokeWidth + paddingTop, w - halfOfStrokeWidth - paddingRight - offset, h - halfOfStrokeWidth - paddingBottom);
        } else {
            offset = (h - w) / 2f;
            mLayer.set(halfOfStrokeWidth + paddingLeft, halfOfStrokeWidth + paddingTop + offset, w - halfOfStrokeWidth - paddingRight, h - halfOfStrokeWidth - paddingBottom - offset);
        }
    ...
    private void drawSearch(Canvas canvas) {
        ...
        if (progress >= stagemMid) {
            ...
            if (progress < stageOne) {
                // 1-1.3
                drawSecondSegment(canvas, curProgress);
            } else if (progress < stageTwo) {
                // 1.3-1.6
