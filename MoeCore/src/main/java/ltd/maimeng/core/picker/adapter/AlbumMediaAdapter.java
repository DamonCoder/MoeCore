...
import ltd.maimeng.core.picker.bean.Album;
import ltd.maimeng.core.picker.bean.IncapableCause;
import ltd.maimeng.core.picker.bean.MediaItem;
import ltd.maimeng.core.picker.bean.SelectedItemCollection;
import ltd.maimeng.core.picker.bean.SelectionSpec;
import ltd.maimeng.core.picker.widget.CheckView;
import ltd.maimeng.core.picker.widget.MediaGridItem;

import androidx.recyclerview.widget.GridLayoutManager;
import androidx.recyclerview.widget.RecyclerView;

public class AlbumMediaAdapter
        extends CursorAdapter<RecyclerView.ViewHolder>
        implements MediaGridItem.OnMediaGridClickListener {
    ...
    public AlbumMediaAdapter(Context context, SelectedItemCollection selectedCollection, RecyclerView recyclerView) {
        ...
        TypedArray ta = context.getTheme().obtainStyledAttributes(new int[]{R.attr.media_picker_item_placeholder});
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == VIEW_TYPE_CAPTURE) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.holder_photo_capture_item, parent, false);
            ...
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.holder_media_grid_item, parent, false);
    ...
    protected void onBindViewHolder(final RecyclerView.ViewHolder holder, Cursor cursor) {
        if (holder instanceof CaptureViewHolder) {
            ...
            TypedArray ta = holder.itemView.getContext().getTheme().obtainStyledAttributes(new int[]{R.attr.media_picker_capture_textColor});
            ...
            mediaViewHolder.mMediaGrid.preBindMedia(new MediaGridItem.PreBindInfo(
    ...
    private void setCheckStatus(MediaItem item, MediaGridItem mediaGrid) {
    ...
    private void updateSelectedItem(MediaItem item, RecyclerView.ViewHolder holder) {
        if (mSelectionSpec.countable) {
            int checkedNum = mSelectedCollection.checkedNumOf(item);
        ...
        for (int i = first; i <= last; i++) {
            ...
            if (holder instanceof MediaViewHolder) {
                if (cursor.moveToPosition(i)) {
                    setCheckStatus(MediaItem.valueOf(cursor), ((MediaViewHolder) holder).mMediaGrid);
                }
            }
        }
    }
    ...
    private int getImageResize(Context context) {
        if (mImageResize == 0) {
            ...
            int availableWidth = screenWidth - context.getResources().getDimensionPixelSize(R.dimen.media_picker_media_grid_spacing) * (spanCount - 1);
    ...
    private static class MediaViewHolder extends RecyclerView.ViewHolder {
        ...
        private MediaGridItem mMediaGrid;

        MediaViewHolder(View itemView) {
            super(itemView);
            mMediaGrid = (MediaGridItem) itemView;
        }
    }
